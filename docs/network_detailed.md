...
> 作者：[@好神奇](https://zjuers.com/rd?url=https://www.cc98.org/user/id/658113)
...
录取通知书附带报到手册中“二、网上注册”部分第一个二维码为“个人自助服务系统”二维码，扫描后进入**浙江大学统一身份认证**界面。亦可点击访问[该网址](https://zjuam.zju.edu.cn/zjuam-main/)。

输入通行证账号和初始密码，登录个人自助服务系统。

- 账号：即个人学号
- 初始密码：Bk+证件号后4位+Txnh。内地学生为身份证，港澳学生为港澳居民来往内地通行证，台湾学生为台湾居民来往大陆通行证。
- 密码示例：Bk1234Txnh（包括数字和英文字符共10位）
...
关于邮件系统和邮件礼仪，可以查看本指引的[“学习-邮件礼仪”](./learning/email_etiquette.md)部分。
...
- 如希望访问其他网页，可以将网页原地址复制下来，如：https://www.cc98.org ，粘贴至中间的“自定义域名”处，选取相应的https模式，回车键入即可访问。
...
- 如果是`http`协议，删去网址中的`webvpn`和`:8001`，将多余的`-`转为`.`且`:8001`后的后缀保持不变
- 如果是`https`协议，删去网址中的`s`、`webvpn`和`:8001`，将多余的`-`转为`.`且`:8001`后的后缀保持不变
